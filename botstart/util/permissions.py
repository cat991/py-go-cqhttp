...
def addPermissions(guild_id,channel_id,user_id:str,day:int):
    ...
    try:
        ...
        with open(beneath, 'w', encoding='utf-8') as f1:
            ...
            f1.close()
            GuildEntity.send_guild_channel_msg(guild_id, channel_id, '添加成功，到期时间:' + time.strftime("%Y年%m月%d日", time.localtime(expiration)))
...
def permissions(data):
    ...
    if message[:4] == '添加管理' and user_id == '144115218676755577':
